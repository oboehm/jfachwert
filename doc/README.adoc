...
|./gradlew -Dorg.gradle.jvmargs="-Xmx768m -XX:MaxMetaspaceSize=512m" clean assemble jar sourceJar javadocJar sign --no-configuration-cache
...
|Damit werden die Artifkate `sourceJar`, `javadocJar` und `jar` erstellt und lokal nach $HOME/.m2/repository/de/jfachwert kopiert

|Bundle für Upload erstellen (manuell)
|`./gradlew generateChecksums --no-configuration-cache`

`cd $HOME/.m2/repository; tar czvf ~/Downloads/jfachwert.tgz `find de/jfachwert -name "\*-6.8.0*"``
...
|`git commit` ...
|NamensKonvention für Tag ist `jfachwert-6.8.0`
...
|`./gradlew asciidoctor --no-configuration-cache`
