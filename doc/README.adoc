...
|./gradlew -Dorg.gradle.jvmargs="-Xmx768m -XX:MaxMetaspaceSize=512m" clean assemble jar sourceJar javadocJar sign
...
`cd $HOME/.m2/repository; tar czvf ~/Downloads/jfachwert.tgz `find de/jfachwert -name "\*-6.7.1*"``
...
|`git commit` ...
|NamensKonvention für Tag ist `jfachwert-6.7.1`
