 ...
 */
public class Mod10Verfahren implements PruefzifferVerfahren<Integer> {
     ...
     */
    @Override
    public boolean isValid(Integer wert) {
        if (wert < 10) {
            return false;
        } else {
            return getPruefziffer(wert).equals(berechnePruefziffer(wert / 10));
        }
    }
     ...
     */
    @Override
    public Integer getPruefziffer(Integer wert) {
        return wert % 10;
    }
     ...
     */
    public Integer berechnePruefziffer(Integer wert) {
        int sum = getQuersumme(wert);
        return sum % 10;
    }

    private static int getQuersumme(Integer wert) {
        char[] digits = Integer.toString(wert).toCharArray();
        int sum = 0;
