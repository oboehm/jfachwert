...
* validation-api: Basis-Bibliothek für Validierung (nur bis v5); ab v6.8 wird `jakarta.validation-api` wieder benötigt, aber nur im eigenen validation-Modul
...
Mit v6.8 wurde die Abhängigkeit zu `jakarta.validation-api` wieder aufgenommen, allerdings isoliert im eigenen validation-Modul.
Damit bleibt der Rest von jFachwert frei von dieser Abhängigkeit; wer die Jakarta-Validierungs-API nicht benötigt, lässt das validation-Modul einfach weg.
