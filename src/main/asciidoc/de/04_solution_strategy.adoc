...
Mit dem validation-Modul, das mit v6.8 hinzugekommen ist, steht zusätzlich ein `FachwertValidator` zur Verfügung, der das `jakarta.validation.Validator`-Interface implementiert.
Erzeugt wird dieser über die `FachwertValidatorFactory`, die das `ValidatorFactory`-Interface implementiert.
Damit können Fachwerte auch über die Standard-API von Jakarta Validation (Bean Validation) validiert werden.
